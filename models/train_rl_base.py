...
Rule-based RL training for mathematical reasoning with binary rewards.
...
Usage:
    python models/train_rl_base.py --config configs/rl_grpo_config.yaml
    python models/train_rl_base.py --config configs/rl_grpo_config.yaml --model_path <your_sft_checkpoint>
"""
...
class GRPOConfig:
    ...
    output_dir: str = "results/rl_grpo_checkpoints"
...
def load_config_from_yaml(config_path: str) -> GRPOConfig:
    ...
    flat_config['output_dir'] = train.get('output_dir', 'results/rl_grpo_checkpoints')
    flat_config['seed'] = train.get('seed', 42)
    flat_config['bf16'] = train.get('bf16', True)  # Use bfloat16 by default
...
def format_prompt(question: str, tokenizer=None) -> str:
    """Format question into prompt using chat template if tokenizer provided"""
...
class GRPOTrainer:
    """Custom GRPO Trainer for mathematical reasoning"""
    ...
    def generate_responses(self, prompts: List[str]) -> List[List[str]]:
        """Generate multiple responses for each prompt (batched for efficiency)"""
        self.model.eval()
        ...
        with torch.no_grad():
            # Batch generation: outputs shape = (batch_size * num_return_sequences, seq_len)
        ...
        # Get prompt lengths for each sample (accounting for padding)
        prompt_lengths = inputs['attention_mask'].sum(dim=1).tolist()
        
        # Decode responses and organize by prompt
        # Output order: [prompt0_seq0, prompt0_seq1, prompt1_seq0, prompt1_seq1, ...]
        all_responses = []
        for i in range(batch_size):
            ...
            for j in range(num_return):
                ...
                # Remove prompt tokens (only keep generated part)
    ...
    def compute_log_probs_batch(self, prompts: List[str], responses: List[str], model) -> torch.Tensor:
        ...
        Args:
            ...
            model: Model to use (self.model or self.ref_model)
        
        Returns:
            Tensor of mean log probs for each response
        """
        batch_size = len(prompts)
        
        # Tokenize prompts to get prompt lengths (without padding for accurate length)
        prompt_lengths = []
        for prompt in prompts:
            prompt_ids = self.tokenizer(prompt, return_tensors="pt", truncation=True, max_length=512)
            prompt_lengths.append(prompt_ids['input_ids'].shape[1])
        
        # Tokenize full texts (prompt + response) with padding
        ...
        for i in range(batch_size):
            ...
            response_logits = logits[i, prompt_len-1:seq_len-1, :]  # Shift by 1 for causal LM
    ...
    def compute_log_probs(self, prompts: List[str], responses: List[str]) -> torch.Tensor:
        """Compute log probabilities of responses using policy model"""
    ...
    def train_step(self, batch: Dict, accumulate: bool = False) -> Dict[str, float]:
        ...
        Args:
            batch: Dict with 'question' and 'ground_truth' keys
            accumulate: If True, don't call optimizer.step() (for gradient accumulation)
        """
        # Extract batch data
        questions = batch['question']
        ground_truths = batch['ground_truth']
        
        # Format prompts - MUST use chat template to match training format
        prompts = [format_prompt(q, self.tokenizer) for q in questions]
        
        # Generate responses (multiple per prompt)
        ...
        for prompt, responses, truth in zip(prompts, all_responses, ground_truths):
        ...
        # Whiten rewards (normalize within batch)
        if self.config.whiten_rewards and len(rewards) > 1:
            rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-8)
        
        # Compute KL penalty (skip if kl_coef is 0 to save compute)
        if self.config.kl_coef > 0:
            kl_penalty = self.compute_kl_penalty(flat_prompts, flat_responses)
        else:
            kl_penalty = torch.tensor(0.0, device=self.device)
        
        # Compute policy gradient loss (BATCHED)
        # REINFORCE: minimize -E[reward * log_prob]
        # We must only compute loss on the RESPONSE tokens, not the prompt!
        self.model.train()
        ...
        # Batch tokenize all full texts (prompt + response)
        ...
        # Create labels: mask prompt tokens and padding with -100
        ...
        # Compute per-sample loss (CrossEntropyLoss with reduction='none')
        ...
        per_token_loss = loss_fct(
            ...
            shift_labels.view(-1)
        ).view(batch_size, -1)  # (batch_size, seq_len-1)
        
        # Average loss per sample (only over non-masked tokens)
        valid_tokens = (shift_labels != -100).float()
        per_sample_loss = (per_token_loss * valid_tokens).sum(dim=1) / (valid_tokens.sum(dim=1) + 1e-8)
        
        # Apply rewards: Policy gradient loss = reward * loss
        # Positive reward -> decrease loss (increase probability)
        # Negative reward -> increase loss (decrease probability)
        ...
        # Only do optimizer step if not accumulating
        if not accumulate:
            # Clip gradients
        ...
        avg_reward = torch.tensor([compute_reward(r, t)[0] for r, t in zip(flat_responses, flat_truths)]).mean().item()
        
        metrics = {
            'loss': final_loss.item() * (self.config.gradient_accumulation_steps if accumulate else 1),  # Unscale for logging
            'reward': avg_reward,
            'accuracy': accuracy,
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for batch_idx in progress_bar:
                ...
                # Run evaluation every eval_steps (skip step 0)
                ...
                if self.config.max_steps > 0 and self.global_step >= self.config.max_steps:
                    ...
                    # Run final evaluation if not just evaluated
            ...
            # Run evaluation at epoch end (if not just evaluated)
...
def main():
    ...
    parser.add_argument('--num_return_sequences', type=int, default=None, help='Number of generations per prompt (default: 2)')
    ...
    parser.add_argument('--skip_save', action='store_true', help='Skip saving checkpoints (eval only mode)')
    ...
    if args.kl_coef is not None:  # Use 'is not None' to allow kl_coef=0
    ...
    if args.use_wandb:
        try:
            import wandb
            run_name = args.wandb_run_name if args.wandb_run_name else f"grpo_rl_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            wandb.init(
    ...
    logger.info("="*80)
    logger.info("GRPO RL Training Configuration")
    logger.info("="*80)
    ...
    # Load reference model (frozen copy) - only if KL penalty is used
    if config.kl_coef > 0:
