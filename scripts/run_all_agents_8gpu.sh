#!/bin/bash
# Master script to run 8 agents on 8 GPUs in tmux
...
for agent in "${!AGENT_GPU_MAP[@]}"; do
    ...
    if [ "$agent" = "majority_vote" ]; then
        ...
        gsm8k_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'gsm8k' --count $COUNT --num_runs $NUM_RUNS --detailed 'false' --save_interval 50"
        math_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'math' --count $COUNT --num_runs $NUM_RUNS --detailed 'false' --save_interval 50"
    elif [ "$agent" = "plan_and_reflection" ]; then
        # plan_and_reflection uses both --max_iterations and --max_subproblems
        gsm8k_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'gsm8k' --count $COUNT --max_iterations $MAX_ITERATIONS --max_subproblems $MAX_SUBPROBLEMS --detailed 'false' --save_interval 50"
        math_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'math' --count $COUNT --max_iterations $MAX_ITERATIONS --max_subproblems $MAX_SUBPROBLEMS --detailed 'false' --save_interval 50"
    else
        # Other agents use standard --max_iterations
        gsm8k_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'gsm8k' --count $COUNT --max_iterations $MAX_ITERATIONS --detailed 'false' --save_interval 50"
        math_cmd="python -m evaluation.eval_agent --model '$MODEL' --agent '$agent' --round '$ROUND' --dataset 'math' --count $COUNT --max_iterations $MAX_ITERATIONS --detailed 'false' --save_interval 50"
    fi
    ...
    tmux send-keys -t $session_name "echo '========================================'" C-m
    tmux send-keys -t $session_name "$gsm8k_cmd" C-m
    
    # Then run math
    tmux send-keys -t $session_name "echo '========================================'" C-m
    tmux send-keys -t $session_name "echo 'Starting $agent on GPU $gpu - MATH'" C-m
    tmux send-keys -t $session_name "echo '========================================'" C-m
    tmux send-keys -t $session_name "$math_cmd" C-m
