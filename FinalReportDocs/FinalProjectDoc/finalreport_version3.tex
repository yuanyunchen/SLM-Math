...
We selected Qwen2.5-Math-1.5B~\cite{yang2024qwen2} after experimenting with alternatives. General-purpose models (Qwen2.5-1.5B, Qwen2.5-0.6B) showed limited responsiveness to agentic workflows, while larger models (Qwen3-1.7B) left little room for improvement. Qwen2.5-Math-1.5B offered a middle ground: small enough to benefit from our methods, yet strong enough for multi-step reasoning.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{llcccc}
        ...
        Solver Checker with Tools & SFT-LoRA & \cmark & \xmark & 81.4\% & 49.8\% \\
        Majority Vote & SFT-LoRA & \cmark & \xmark & 70.2\% & 54.8\% \\
        Agent with Python Tools & SFT-LoRA & \xmark & \xmark & 72.6\% & 45.2\% \\
        \midrule
        ...
        Solver--Verifier (SFT+RL) & Solver: RL, Verifier: RL & \cmark & \cmark & 86.8\%$^\ast$ & 68.8\%$^\ast$ \\
        \midrule
...
\textbf{Teacher Model Dependence.} Our data generation pipeline requires access to Grok-4.1 and MiniMax-M2 APIs, which may not be universally available. Alternative teacher models may yield different data quality and downstream performance.
...
\textbf{Roger Wang} led data pipeline development including the two-round chain-of-thought generation system with Grok-4.1 and MiniMax-M2, designed and implemented the Solver--Verifier agent architectures, and conducted comprehensive evaluation experiments across all agent variants.
