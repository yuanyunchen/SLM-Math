#!/usr/bin/env python3
"""
Reasoning + Code Data Generation Script
---------------------------------------
...
Output focuses on reasoning plus executable Python code. The code result is
treated as the source of truth for the final answer (the boxed answer must
match the code output).

Usage:
    python -m dataset.build_reasoning_code_data \\
...
def get_system_prompt(thinking_effort: str) -> str:
    ...
    effort_configs = {
        "low": {
            "instruction": (
                "Solve the math problem. Provide concise reasoning and minimal, "
                "readable Python code that computes the answer."
            ),
            "thinking_tokens": 300
        },
        "medium": {
            "instruction": (
                "Solve the math problem step by step. Provide clear reasoning and "
                "Python code that deterministically computes the answer. Keep the "
        ...
        "high": {
            "instruction": (
                "Solve the math problem with detailed reasoning and provide "
                "well-structured Python code that computes and prints the final "
                "answer. Include sanity checks when helpful."
            ),
            "thinking_tokens": 1400
        },
        "very_high": {
            "instruction": (
                "Provide comprehensive reasoning and robust Python code that "
                "computes the answer. Favor clarity, determinism, and correctness."
            ),
...
Output format:
<think>...concise but clear reasoning...</think>
<code>
```python
# Use only Python 3.9 standard library (math, fractions, decimal, itertools allowed).
# Do not read/write files or make network/system calls.
# Set `final_answer` to the numeric/expr result and print it.
...
print(final_answer)
```
</code>
<final>Therefore, the answer is \\boxed{{...}}</final>

Rules:
- The Python code must be executable as-is and deterministically compute the answer.
- The printed value and `final_answer` must match the boxed answer; the code result is the source of truth.
- Prefer integers, simplified fractions, or short decimals over verbose text.
- Avoid randomness unless seeded; avoid lengthy brute force."""
...
def process_datasets(
    ...
    base_name = f"reasoning_code_{dataset_str}_{timestamp}"
...
def save_final_results(
    ...
    with open(stats_file, "w", encoding="utf-8") as f:
        f.write("="*80 + "\n")
        f.write("Reasoning + Code Data Generation Statistics\n")
        f.write("="*80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Reasoning + Code Data Generation with Multi-Model Support"
    )
    ...
    print("\n" + "="*80)
    print("REASONING + CODE DATA GENERATION")
    print("="*80)
